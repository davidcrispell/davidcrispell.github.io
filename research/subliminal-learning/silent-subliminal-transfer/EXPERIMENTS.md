...
## 2026-09-03 exploratory update — Pythia-recipe transplant and AdamW beta2 screens

These runs use the same Gemma-2-9B-IT student and a system-prompted Gemma
teacher, but transplant the Pythia project's one-pass example-indexed schedule,
optimizer geometry, and cloze evaluation. Every arm reuses one byte-identical
8,192-example carrier block restricted to digits, commas, and spaces. They are
one-paired-seed exploratory hill-climbing runs and support no population-level
optimizer optimum.

- At EB16/r8/alpha16, changing only `adam_beta2` from 0.95 to 0.92 increased
  the update-512 paired cloze-margin effect from **+0.69471** to **+0.74031**
  and the paired wolf-candidate-probability effect from **+0.07512** to
  **+0.08654**. Beta2=0.92 did not dominate the trajectory: 0.95 was stronger
  at updates 64 and 128.
- At EB8/r8/alpha32, one-pass endpoints were:

  | beta2 | paired margin | paired wolf probability |
  | ---: | ---: | ---: |
  | 0.90 | +0.74084 | +0.09554 |
  | 0.92 | +0.72833 | +0.09378 |
  | 0.95 | **+1.00918** | **+0.13037** |

  Beta2=0.95 was weakest at update 512 (+0.4477) before becoming strongest at
  update 1,024. Treatment training loss ranged only 0.3892–0.3895 and control
  loss 0.6085–0.6092 across the three endpoints, despite the roughly 39%
  spread in margin transfer. This is descriptive evidence that optimizer path
  can change the behavioral contrast without a comparable carrier-loss change;
  it is not evidence that 0.95 is globally optimal.
- Canonical artifacts are the sibling
  `runs/wolf-sl-gemma2-9b-pythia-eb{16,8}-*/evaluations/cloze/trajectory_summary.json`
  files and the EB16 beta2=0.95
  `data/pythia_transplant_data_verification.json`.

## 2026-09-03 exploratory update — treatment-only continuation and probe disagreement

The EB8/r8/alpha32/beta2=0.95 treatment adapter was continued without a
matched control beyond one pass. Its treatment-minus-frozen-base cloze effect
was nonmonotone, peaking post hoc at 6.5 passes (margin **+2.6874**, candidate
probability **+0.3152**) and ending at ten passes at +1.9146/+0.2229. These are
total treatment-versus-base changes, not causal treatment-control SL effects.

At the selected 6.5-pass checkpoint, the frozen ten-animal cloze assigned wolf
probability 0.3797 and dog probability 0.1489; wolf ranked first on 51.7% of
prompts. A separate positive-temperature free-response assay produced a starkly
different readout. The frozen first-recognized-animal heuristic classified
857/1,600 responses as dog and 3/1,600 as wolf (dog was 83.4% of recognized-
animal responses); greedy decoding also produced a dog answer. The same
heuristic found no recognized animal in any of 1,600 base responses. These
counts are heuristic classifications, not human-verified declarations of
preference. They establish probe disagreement at one adaptively selected,
treatment-only checkpoint, not a new causal trait-transfer estimate.

Canonical artifacts:
`runs/wolf-sl-gemma2-9b-pythia-eb8-alpha32-beta95-tenpass-treatment-pilot-v1/evaluations/cloze/treatment_base_trajectory_summary.json`
and
`runs/wolf-sl-gemma2-9b-pythia-eb8-alpha32-beta95-tenpass-treatment-pilot-v1/evaluations/direct_favorite_animal/checkpoint-6656/v1/summary.json`.
